...
function renderOthers() {
  ...
  for (let key in others) {
    ...
    otherDiv.innerHTML =
      "<p>" + other.userName + "</p><img src='" + other.base64 + "' />";
  }
}
...
function getNormalized2DDistance(me, others) {
  ...
  for (let key in others) {
    let other = others[key];
    other.normalizedDistance =
      (other.distance - minDistance) / (maxDistance - minDistance);
...
async function askForEmbedding(prompt, base64) {
  ...
  if (replicateJSON.output.length == 0) {
    ...
    let userName = user.displayName
      ? user.displayName
      : user.email.split("@")[0];
    FB.setDataInFirebase(exampleName + "/" + user.uid, {
      userName: userName,
      prompt: prompt,
      base64: base64,
